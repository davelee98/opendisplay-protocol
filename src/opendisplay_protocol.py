# @generated by tools/gen_python_protocol.py -- DO NOT EDIT BY HAND.
...
# Regenerate: tools/gen_python_protocol.py --write   (CI drift gate: --check)
"""OpenDisplay BLE wire-protocol constants.

Generated from the canonical opendisplay_protocol.h (protocol v2.2). Every
value here travels on the OpenDisplay BLE wire. Do not hand-edit; edit the
header and regenerate. Constants are flat (not an IntEnum) because the wire
intentionally reuses byte values across distinct names -- an enum would alias
and hide one. NACK error codes are opcode-SCOPED: decode data[0] only in the
...
OD_PROTOCOL_VERSION_MAJOR: Final = 2
OD_PROTOCOL_VERSION_MINOR: Final = 2
OD_PROTOCOL_VERSION_STR: Final = "2.2"
...
# @limits:   -
# @targets:  Firmware | NRF54 | Silabs | NRF52811
CMD_FIRMWARE_VERSION: Final = 0x0043
...
# @targets:  Firmware | NRF54 | Silabs | NRF52811
CMD_DIRECT_WRITE_START: Final = 0x0070
...
# @targets:  Firmware | NRF54 | Silabs | NRF52811
CMD_DIRECT_WRITE_DATA: Final = 0x0071
...
# @limits:   -
# @targets:  Firmware | NRF54 | Silabs | NRF52811
CMD_DIRECT_WRITE_END: Final = 0x0072
...
CMD_PARTIAL_WRITE_START: Final = 0x0076
...
CMD_PIPE_WRITE_START: Final = 0x0080
...
# @targets:  Firmware      (NOT NRF54, NOT Silabs, NOT NRF52811)
CMD_PIPE_WRITE_DATA: Final = 0x0081
...
CMD_PIPE_WRITE_END: Final = 0x0082
...
RESP_ACK: Final = 0x00  # success
RESP_NACK: Final = 0xFF  # failure; data[0] often an error code
RESP_AUTH_REQUIRED: Final = 0xFE  # gated command sent without a live session

# Command-echo bytes (frame position 1) = low byte of the matching CMD_*.
RESP_CONFIG_READ: Final = 0x40
RESP_CONFIG_WRITE: Final = 0x41
RESP_CONFIG_CHUNK: Final = 0x42
RESP_FIRMWARE_VERSION: Final = 0x43
RESP_MSD_READ: Final = 0x44
RESP_CONFIG_CLEAR: Final = 0x45
RESP_AUTHENTICATE: Final = 0x50
RESP_ENTER_DFU: Final = 0x51
RESP_POWER_OFF: Final = 0x52
RESP_DEEP_SLEEP: Final = 0x53

# Direct-write family (Firmware / NRF52811 + shared).
RESP_DIRECT_WRITE_START_ACK: Final = 0x70
RESP_DIRECT_WRITE_DATA_ACK: Final = 0x71
RESP_DIRECT_WRITE_END_ACK: Final = 0x72
RESP_DIRECT_WRITE_REFRESH_SUCCESS: Final = 0x73  # unsolicited device->host: panel refreshed OK
RESP_DIRECT_WRITE_REFRESH_TIMEOUT: Final = 0x74  # unsolicited device->host: refresh timed out
RESP_DIRECT_WRITE_ERROR: Final = 0xFF  # direct-write NACK status (== RESP_NACK)

# LED / buzzer acks.
RESP_LED_ACTIVATE_ACK: Final = 0x73
RESP_LED_STOP_ACK: Final = 0x75
RESP_BUZZER_ACK: Final = 0x77

# NFC endpoint echo byte (was 0x82u in protocol v1; see CMD_NFC_ENDPOINT).
RESP_NFC_ENDPOINT: Final = 0x83
...
AUTH_STATUS_CHALLENGE: Final = 0x00
AUTH_STATUS_SUCCESS: Final = AUTH_STATUS_CHALLENGE  # alias: step-2 success == challenge value
AUTH_STATUS_FAILED: Final = 0x01  # wrong key
AUTH_STATUS_ALREADY: Final = 0x02  # already authenticated
AUTH_STATUS_NOT_CONFIG: Final = 0x03  # encryption not configured
AUTH_STATUS_RATE_LIMIT: Final = 0x04  # >=10 attempts / 60 s
AUTH_STATUS_ERROR: Final = 0xFF  # generic error
...
OD_ERR_PARTIAL_ETAG_MISMATCH: Final = 0x01  # old_etag != displayed etag
OD_ERR_PARTIAL_RECT_OOB: Final = 0x03  # rectangle out of panel bounds
OD_ERR_PARTIAL_RECT_ALIGN: Final = 0x04  # x / width not a multiple of 8
OD_ERR_PARTIAL_FLAGS: Final = 0x05  # bad / unsupported flags
OD_ERR_PARTIAL_STREAM: Final = 0x06  # stream / length error
OD_ERR_PARTIAL_UNSUPPORTED: Final = 0x07  # partial write unsupported (e.g. not 1bpp)
...
OD_ERR_PIPE_START_BAD_HEADER: Final = 0x01  # bad length or version
OD_ERR_PIPE_START_UNKNOWN_FLAG: Final = 0x02  # unknown flags bit set
OD_ERR_PIPE_START_SIZE_MISMATCH: Final = 0x03  # total_size inconsistent
OD_ERR_PIPE_START_ETAG_MISMATCH: Final = 0x05  # partial: old_etag mismatch
OD_ERR_PIPE_START_PARTIAL_UNSUPPORTED: Final = 0x06  # partial mode not supported
OD_ERR_PIPE_START_RECT_INVALID: Final = 0x07  # partial: invalid rectangle
...
OD_ERR_DEEP_SLEEP_UNSUPPORTED: Final = 0x00  # target has no deep sleep (nRF)
OD_ERR_DEEP_SLEEP_DISABLED: Final = 0x01  # deep_sleep_time_seconds == 0 in config
OD_ERR_DEEP_SLEEP_NOT_BATTERY: Final = 0x02  # power_mode != 1 (mains-powered)

# 4d. POWER-OFF errors -- scope: CMD_POWER_OFF (0x0052) ONLY.
#     NACK frame: [0xFF][0x52][err][0x00].
OD_ERR_POWER_OFF_UNSUPPORTED: Final = 0x00  # no D-FF power latch on this target

# ===== Section 5: NFC SUB-PROTOCOL (rides CMD_NFC_ENDPOINT 0x0083) =====

# NFC sub-command (request payload byte 0).
NFC_SUB_READ: Final = 0x00
NFC_SUB_WRITE: Final = 0x01  # single-shot write
NFC_SUB_WRITE_START: Final = 0x10  # begin chunked write
NFC_SUB_WRITE_DATA: Final = 0x11  # append chunk
NFC_SUB_WRITE_END: Final = 0x12  # commit chunked write

# NFC record types (rec_type field). Wire bytes an agent must ground on --
# kept here (not in per-repo constants) for that reason.
OD_NFC_REC_TEXT: Final = 0
OD_NFC_REC_URI: Final = 1
OD_NFC_REC_WELL_KNOWN_RAW: Final = 2
OD_NFC_REC_MIME: Final = 3
OD_NFC_REC_RAW_NDEF: Final = 4

# NFC IC (tag chip) selector.
OD_NFC_IC_AUTO: Final = 0
OD_NFC_IC_TNB132M: Final = 1
...
# 0x0082 NFC opcode.
NFC_STATUS_READ_DATA: Final = 0x80  # [.. 0x80][rec_type][len:2 BE][data]
NFC_STATUS_WRITE_COMMITTED: Final = 0x81  # single write / chunk END committed
NFC_STATUS_CHUNK_ACCEPTED: Final = 0x82  # WRITE_START or WRITE_DATA accepted

# NFC NACK error codes (4th byte of [0xFF][0x83][0xFF][err]).
NFC_ERR_MALFORMED: Final = 0x01  # malformed / short frame
NFC_ERR_READ_FAILED: Final = 0x02  # tag read failed
NFC_ERR_TAG_WRITE_FAILED: Final = 0x03  # tag write failed
NFC_ERR_UNKNOWN_SUBCMD: Final = 0x04  # unknown sub-command byte
NFC_ERR_INVALID_REC_TYPE: Final = 0x05  # rec_type not in OD_NFC_REC_*
NFC_ERR_BAD_TOTAL_LEN: Final = 0x06  # total_len == 0 or > 512
NFC_ERR_CHUNK_NO_START: Final = 0x07  # DATA/END without active START (or wrong connection)
NFC_ERR_CHUNK_OVERFLOW: Final = 0x08  # chunk would exceed total_len
NFC_ERR_END_LEN_MISMATCH: Final = 0x09  # END with received != total_len

# ===== Section 6: PIPE SLIDING-WINDOW WIRE CONSTANTS (0x0080..0x0082) =====

PIPE_VERSION: Final = 0x01  # carried in 0x80 request/response ver field
PIPE_FLAG_COMPRESSED: Final = 0x01  # flags bit0: streamed bytes are zlib-compressed
PIPE_FLAG_PARTIAL: Final = 0x02  # flags bit1: partial-region refresh
PIPE_MAX_FRAME: Final = 244  # max on-wire frame size (GATT write ceiling)
PIPE_FRAME_OVERHEAD: Final = 3  # plaintext 0x81 header: cmd(2) + seq(1)
PIPE_ACK_MASK_BITS: Final = 32  # SACK ack_mask width in bits
OD_PIPE_MAX_PAYLOAD: Final = 244  # max payload buffer used by pipe/NFC responses

# ===== Section 7: CHUNK / SIZE BUDGETS =====

OD_BLE_MAX_FRAME: Final = 256  # max BLE ATT MTU: opcode(1) + handle(2) + value, inclusive; usable single-write value is OD_BLE_MAX_FRAME - 3 = 253 (BLE peer of OD_LAN_MAX_FRAME)
CONFIG_CHUNK_SIZE: Final = 200  # max config data bytes per chunk
CONFIG_CHUNK_SIZE_WITH_PREFIX: Final = 202  # first chunked CONFIG_WRITE payload: total(2) + 200
MAX_CONFIG_CHUNKS: Final = 20  # upper bound on config chunks in a write
MAX_RESPONSE_DATA_SIZE: Final = 100  # max bytes in a single notification frame
...
BLE_CMD_HEADER_SIZE: Final = 2  # opcode bytes (also the AAD length)
ENCRYPTION_NONCE_SIZE: Final = 16  # envelope nonce (CCM nonce = bytes 3..15)
ENCRYPTION_TAG_SIZE: Final = 12  # CCM authentication tag length
...
OD_LAN_TCP_PORT: Final = 2446  # DEFAULT plaintext port; live port = WifiConfig.server_port (this default if 0)
OD_LAN_TLS_PORT: Final = 2447  # DEFAULT TLS-PSK port; DERIVED live = server_port + 1 (== OD_LAN_TCP_PORT + 1)
OD_LAN_MAX_FRAME: Final = 4096  # max WIRE frame: [len:2 LE] prefix + payload, inclusive
OD_LAN_MAX_PAYLOAD: Final = 4094  # max payload bytes after the prefix; == OD_LAN_MAX_FRAME - 2
OD_LAN_MDNS_SERVICE: Final = "_opendisplay._tcp"  # DNS-SD service; FQDN "_opendisplay._tcp.local."
OD_LAN_READ_TIMEOUT_S: Final = 30  # idle timeout: drop a client after this many seconds of no traffic

__all__ = [
    "OD_PROTOCOL_VERSION_MAJOR",
    "OD_PROTOCOL_VERSION_MINOR",
    "OD_PROTOCOL_VERSION_STR",
    "CMD_REBOOT",
    "CMD_CONFIG_READ",
    "CMD_CONFIG_WRITE",
    "CMD_CONFIG_CHUNK",
    ...
    "CMD_POWER_OFF",
    "CMD_DEEP_SLEEP",
    "CMD_DIRECT_WRITE_START",
    "CMD_DIRECT_WRITE_DATA",
    "CMD_DIRECT_WRITE_END",
    "CMD_LED_ACTIVATE",
    "CMD_LED_STOP",
    "CMD_PARTIAL_WRITE_START",
    "CMD_BUZZER",
    "CMD_PIPE_WRITE_START",
    "CMD_PIPE_WRITE_DATA",
    "CMD_PIPE_WRITE_END",
    "CMD_NFC_ENDPOINT",
    "RESP_ACK",
    ...
    "RESP_POWER_OFF",
    "RESP_DEEP_SLEEP",
    "RESP_DIRECT_WRITE_START_ACK",
    "RESP_DIRECT_WRITE_DATA_ACK",
    "RESP_DIRECT_WRITE_END_ACK",
    "RESP_DIRECT_WRITE_REFRESH_SUCCESS",
    "RESP_DIRECT_WRITE_REFRESH_TIMEOUT",
    "RESP_DIRECT_WRITE_ERROR",
    "RESP_LED_ACTIVATE_ACK",
    "RESP_LED_STOP_ACK",
    "RESP_BUZZER_ACK",
    "RESP_NFC_ENDPOINT",
    ...
    "AUTH_STATUS_FAILED",
    "AUTH_STATUS_ALREADY",
    "AUTH_STATUS_NOT_CONFIG",
    "AUTH_STATUS_RATE_LIMIT",
    "AUTH_STATUS_ERROR",
    "OD_ERR_PARTIAL_ETAG_MISMATCH",
    "OD_ERR_PARTIAL_RECT_OOB",
    "OD_ERR_PARTIAL_RECT_ALIGN",
    "OD_ERR_PARTIAL_FLAGS",
    "OD_ERR_PARTIAL_STREAM",
    "OD_ERR_PARTIAL_UNSUPPORTED",
    "OD_ERR_PIPE_START_BAD_HEADER",
    "OD_ERR_PIPE_START_UNKNOWN_FLAG",
    "OD_ERR_PIPE_START_SIZE_MISMATCH",
    "OD_ERR_PIPE_START_ETAG_MISMATCH",
    "OD_ERR_PIPE_START_PARTIAL_UNSUPPORTED",
    "OD_ERR_PIPE_START_RECT_INVALID",
    "OD_ERR_DEEP_SLEEP_UNSUPPORTED",
    "OD_ERR_DEEP_SLEEP_DISABLED",
    "OD_ERR_DEEP_SLEEP_NOT_BATTERY",
    "OD_ERR_POWER_OFF_UNSUPPORTED",
    "NFC_SUB_READ",
    "NFC_SUB_WRITE",
    "NFC_SUB_WRITE_START",
    "NFC_SUB_WRITE_DATA",
    ...
    "OD_NFC_REC_TEXT",
    "OD_NFC_REC_URI",
    "OD_NFC_REC_WELL_KNOWN_RAW",
    "OD_NFC_REC_MIME",
    "OD_NFC_REC_RAW_NDEF",
    "OD_NFC_IC_AUTO",
    "OD_NFC_IC_TNB132M",
    "NFC_STATUS_READ_DATA",
    "NFC_STATUS_WRITE_COMMITTED",
    "NFC_STATUS_CHUNK_ACCEPTED",
    "NFC_ERR_MALFORMED",
    "NFC_ERR_READ_FAILED",
    "NFC_ERR_TAG_WRITE_FAILED",
    "NFC_ERR_UNKNOWN_SUBCMD",
    "NFC_ERR_INVALID_REC_TYPE",
    "NFC_ERR_BAD_TOTAL_LEN",
    "NFC_ERR_CHUNK_NO_START",
    "NFC_ERR_CHUNK_OVERFLOW",
    "NFC_ERR_END_LEN_MISMATCH",
    "PIPE_VERSION",
