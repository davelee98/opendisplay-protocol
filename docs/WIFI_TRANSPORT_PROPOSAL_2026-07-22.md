...
**D3-reconciliation** (correctness-critical, gets harder once field entries exist) is the top open decision to close. **D6/D7/D8/D9** are cheap to close with a little investigation. **D2** is now an objective — its remaining work is *implementing* the tuned TLS build and validating on C6 hardware, not a go/no-go. *Resolved:* D1 (no WiFi-only), D2 (TLS-on-C6 is a target), D4 (write-only-secrets no; plaintext serves full control plane), D5 (no concurrent clients), D3-anchor (BLE MAC).
