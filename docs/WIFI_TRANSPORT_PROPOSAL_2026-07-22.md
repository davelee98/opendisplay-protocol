...
- **Optional hardening (plaintext channel scoping) — still open (§8 D4):** on the plaintext 2446 channel you *may* scope which opcodes are served — allow the data plane (DIRECT_WRITE / status) while gating privileged control (`ENTER_DFU`, `CONFIG_WRITE`, `POWER_OFF`, `REBOOT`) to the TLS port / BLE. Bounds the blast radius of an open channel without friction on the draw path.
