...
### 3.2 Network/session opcode band `0x0054–0x005F` — **DEFERRED (not in the initial version)**

**The initial version adds no new opcodes** — see §3 above for why everything it needs already exists. The `0x0054–0x005F` band is **reserved** for a future enhancement; the sketch below is retained for planning only:

| Opcode | Name | Purpose (future) |
|---|---|---|
| `0x0054` | `CMD_NET_STATUS` | Runtime WiFi state (state / IP / RSSI). The high-value one — converts fire-and-pray provisioning into observable provisioning with actionable errors; genuinely not in config (it's *runtime* state, not configured state). |
| `0x0055` | `CMD_NET_JOIN` | Join now without reboot (live reconfiguration); async result. The higher-cost one (async join state machine). |
| `0x0056` | `CMD_NET_FORGET` | Scoped WiFi-creds wipe + drop association; other config untouched. |
| ~~`0x0057`~~ | ~~`CMD_NET_CAPS`~~ | **Likely unnecessary — do not implement.** Transports / mode / port / max-frame / version are already available from `communication_modes` + `encryption_enabled` (config reads), the protocol constants (spec, gated by `pv`), and `CMD_FIRMWARE_VERSION` / the mDNS `fw`/`pv`/`cm`/`tls` TXT. A separate capability command duplicates all of it; leave `0x0057` reserved. |
| `0x0057–0x5F` | reserved | (incl. the former CAPS slot) WS upgrade, static IP, multi-AP. |
...
- **F3 (FUTURE — not in the initial version) — NET opcodes** (`communication.cpp` dispatch): implement `CMD_NET_STATUS` (0x0054) first, then `CMD_NET_JOIN`/`CMD_NET_FORGET`. `NET_STATUS` is the highest-value piece (provisioning feedback, replacing fire-and-pray config+reboot). Do **not** implement `CMD_NET_CAPS` (redundant — §3.2). Deferred behind the config-write+reboot MVP (§8 D8). *Medium.*
...
- **P5 — Provisioning API**: `device.provision_wifi(ssid, password, encryption_type=…)` — patches only the `0x26` TLV + sets comm-modes bit 2 via existing `write_config()`, then reboot to apply. Implementable over BLE today, **no new opcodes**. (A future version adds `CMD_NET_JOIN`/`CMD_NET_STATUS` for join-now + feedback — not in the initial release.)
...
- **H5 — provisioning service** `opendisplay.configure_wifi(device_id, ssid, password, …)`: connect over BLE, `provision_wifi()` (P5), then **reboot**; the tag reappears via mDNS if it joined. (Future: poll `CMD_NET_STATUS` for explicit join success/failure — not in the initial release.)
...
**Phase 0 — Spec (opendisplay-protocol):** SECTION 9 (incl. `OD_LAN_TLS_PORT 2447` + the `encryption_enabled`→port/mode rule) + mDNS TXT spec; gen + sync + CI check. **No new opcodes and no new config packet in the initial version** — the `0x0054–0x005F` NET band is reserved for later (§3.2). Everything downstream implements against this.
...
**Future (post-initial) — Provisioning UX + status:** Firmware F3 (`CMD_NET_STATUS` first, then `JOIN`/`FORGET`; **not** `CMD_NET_CAPS`), py-opendisplay P5/P7, HA H5–H6 for live reconfig + join feedback. **Not part of the initial rollout** — the initial version provisions via config-write + reboot with mDNS discovery and **no NET opcodes** (§8 D8).
